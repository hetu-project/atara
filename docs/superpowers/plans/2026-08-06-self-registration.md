...
`{found && found.id === value ? ...}` 这个条件里的 `value` 判断**必须保留**。本组件是受控的，
`value` 由父级持有；只看 `found` 的话，父级若没接受这个值（或表单被重置），界面仍会显示
「已选择」，与表单里的真实值不符。`value` 这个 prop 存在的意义就在这里，不要把它改成未使用。
Step 1 的测试用一个带 `useState` 的 Harness 模拟了这条受控回路，能正常覆盖到这个分支。
