...
接单后会有一段**全屏 AI 检查演出**（`components/MatchCeremony.tsx`）：
双方卡片汇聚 → 提取特征向量 → 八个模型分四组并行推理、逐条弹出 →
集成评分与置信度 → 自然语言结论逐字打出。右上角可「跳到结果」，
`prefers-reduced-motion` 下直接显示终态。

**演出不会自动关闭**，停在结论页等你选「继续交易」或「放弃这一单」。因此所有
状态写入都推迟到点「继续交易」那一刻——放弃的话挂单原样留在大厅，不会凭空少
一条。操作栏 `sticky` 钉在视口底部，八项检查铺开后按钮也不会掉到折叠线以下。
