...
| `/app/quick` | 快捷交易 | 登录后的落地页。填金额，AI 自动挑对手方 |
| `/app/pool` | AI 撮合大厅 | 挂单卡片墙，点卡片弹 AI 审核 |
| `/app/overview` | 智能总览 | **不在侧边栏里**，只保留路由，直接输 URL 可访问 |
| `/app/trades` | AI 审核记录 | 每笔交易的模型判定结果 |
| `/app/todo` | 待我确认 | 补齐材料后自动重新推理 |
| `/app/desk` | 我的账户 | 买入账户与卖出账户 |

侧边栏分两组：**智能交易**（AI 撮合大厅、快捷交易）和 **账户中心**（AI 审核记录、
待我确认、我的账户）。登录后以及 `/app`、未匹配路径一律落到快捷交易。URL 保留英文（`pool` / `quick` / `trades`），
...
**弹窗不会自动关闭**，停在结论处等你选「继续交易」或「放弃这一单」。因此所有
状态写入都推迟到点「继续交易」那一刻——放弃的话挂单原样留在大厅，不会凭空少
一条。弹窗是固定头部 + 可滚动正文 + 固定底部操作栏，八项检查铺开也不会把按钮
挤出视野。

八项检查分四组，每组挂一个「模型」名（`atara-kyc-v4`、`kyt-trace-3.1`、
`graph-cluster-2.4` 等）并显示单项耗时。分组和模型名是为了让画面读起来像多模型
并行推理，而不是一张写死的规则清单——后者一眼就能看出是 if/else。结论那段话由
实际检查结果拼出，会点名每一个问题项；写死一段模板是「这是假的」最明显的信号。
...
`dist/app/index.html`，否则用户刷新 `/app/trades` 会 404。这是客户端路由的常规要求，
